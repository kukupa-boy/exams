<template>
	...
	:class="{ 
		...
		'uni-collapse-cell--hide':!isOpen 
		 }" class="uni-collapse-cell" :style="{height:!isOpen?itemHeight:'auto'}">
		<view class="uni-collapse-cell__title" @click="onClick(title)" :style="{height:itemHeight}">
			<image v-if="thumb" :src="thumb" class="uni-collapse-cell__title-img" />
			<text class="uni-collapse-cell__title-text" :style="{color:fontColor,fontSize:fontSize}">{{ title }}</text>
			<text v-if="otherText" class="sumText">{{otherText}}</text>
			<!-- #ifdef MP-ALIPAY -->
	...
	export default {
		...
		methods: {
			onClick(value) {
				...
				if (String(this.collapse.accordion) === 'true') {
					if(!this.isOpen){
						this.$emit("getYearMonthData",value)
					}
