<template>
	<view v-if="show" class="mask" :class="rtlClazz" @click="toggleMask" @touchmove.stop.prevent="stopPrevent"
	...
	>
		...
		>
			...
			<view class="bm-content">
				<view class="text-orange text-lg padding-sm">{{i18n.sharetip}}</view>
				<button class="cu-btn bg-gradual-orange shadow-blur" @click="gotoPromoter">{{i18n.sharebtn}}</button>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		computed:{
			...mapState(['hasLogin','promoter','rtlClazz','hasPostAuth']),
			i18n () {
			...
			shareList() {
				return [
					...
					},
					{
						...
						text: this.$t('shareinfo').sharestudent
					}
				];
			}
		},
