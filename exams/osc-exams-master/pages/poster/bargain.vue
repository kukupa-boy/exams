...
<script>
	import { postBargainProduct } from '@/api/share';
	...
	export default {
		...
		computed: {
			...
			i18n () {  
			  return this.$t('poster').bargain
			}
		},
		async onLoad(options) {
			let title = this.$t('poster').bargain.title;
			uni.setNavigationBarTitle({
			　　title: title
			})
			let productid = options?options.productid:null;
			if(productid) {
				let codeInfo = await postBargainProduct(productid,this.langtype);
				if(codeInfo && codeInfo.qrcodeUrl) {
					this.httpCodeUrl = codeInfo.qrcodeUrl;
					this.httpBackgroundImageUrl = codeInfo.backgroundUrl;
				}
			}
			if(this.httpBackgroundImageUrl) {
				this.shareFc();
			}
